...
const REVIEW_ITEMS = ['原意一致', '术语准确', '数据与引用未被改动', '上下文衔接自然'];
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            {phase === 'done' && (
              <div className="space-y-4">
                <div className="custom-scrollbar min-h-[400px] rounded-2xl border border-green-100 bg-green-50/30 p-4 text-[15px] leading-relaxed whitespace-pre-wrap text-gray-800">{result}</div>
                <div className="rounded-2xl border border-gray-100 bg-gray-50 p-3 text-xs text-gray-500">
                  <span className="font-medium text-gray-700">建议对照检查：</span>
                  <span>{REVIEW_ITEMS.join('、')}。</span>
                </div>
              </div>
            )}
          </div>
